...
TEST_F(Rdata_TSIG_Test, fromText) {
    ...
    TSIG tsig1("hmac-md5.sig-alg.reg.int 1286779327 300 "
               "0 16020 BADKEY 0");
...
TEST_F(Rdata_TSIG_Test, createFromParams) {
    ...
    EXPECT_EQ(0, TSIG(valid_text2).compare(
                  TSIG(Name("hmac-sha256"), 1286779327, 300, 12,
                       fake_data, 16020, 16, 0, NULL)));

    const uint8_t fake_data2[] = { 0x14, 0x02, 0x84, 0x14, 0x02, 0x84 };
    EXPECT_EQ(0, TSIG(valid_text3).compare(
                  TSIG(Name("hmac-sha1"), 1286779327, 300, 12,
                       fake_data, 16020, 18, 6, fake_data2)));

    EXPECT_THROW(TSIG(Name("hmac-sha256"), 1ULL << 48, 300, 12,
                      fake_data, 16020, 18, 6, fake_data2),
                 isc::OutOfRange);
    EXPECT_THROW(TSIG(Name("hmac-sha256"), 0, 300, 0, fake_data, 16020,
                      18, 0, NULL),
                 isc::InvalidParameter);
    EXPECT_THROW(TSIG(Name("hmac-sha256"), 0, 300, 12, NULL, 16020,
                      18, 0, NULL),
                 isc::InvalidParameter);
    EXPECT_THROW(TSIG(Name("hmac-sha256"), 0, 300, 0, NULL, 16020,
                      18, 0, fake_data),
                 isc::InvalidParameter);
    EXPECT_THROW(TSIG(Name("hmac-sha256"), 0, 300, 0, NULL, 16020,
                      18, 6, NULL),
...
TEST_F(Rdata_TSIG_Test, compare) {
    ...
    EXPECT_EQ(0, compare_set[0].compare(
                  TSIG("A.EXAMPLE 0 300 0 16020 0 0")));
