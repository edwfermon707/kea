...
public:
    ...
    /// @brief Calls {f} for the element found at {xpath} relative to
    /// {data_node} and sets the result in {storage} with the {xpath} key.
    ///
    /// @tparam T typename of the class owning the function to be called
    /// @tparam TT typename of the translator used, can be the same as T or a base of TT
    ///
    ...
    /// @param tt pointer to the owner of the function
    ...
    /// @brief This is the same as the previous @ref checkAndGet, except
    /// the called function accepts both a data node and an xpath.
    template <typename T, typename TT>
    void checkAndGet(isc::data::ElementPtr const& storage,
                     libyang::DataNode const& data_node,
                     std::string const& xpath,
