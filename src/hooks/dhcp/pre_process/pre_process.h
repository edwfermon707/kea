...
///  - the query packet processing performed by the process method
///
...
public:
    ...
    /// @brief Configure the Pre Process implementation.
    ///
    ...
    void process(isc::dhcp::Option::Universe universe, PktType query) {
        for (auto pair : getOptionConfigMap()) {
            for (const OptionConfigPtr& opt_cfg : pair.second) {
                ...
                isc::dhcp::OptionPtr opt = query->getOption(code);
                ...
                case SUPERSEDE:
                    ...
                    while (query->getOption(code)) {
                        query->delOption(code);
                    }
                    // Add the option.
                    query->addOption(opt);
        ...
        for (auto pair : getSubOptionConfigMap()) {
            for (auto sub_pair : pair.second) {
                ...
                std::string value;
                isc::dhcp::OptionBuffer buffer;
                isc::dhcp::OptionPtr opt = query->getOption(opt_code);
...
/// @brief The type of shared pointers to Pre Process implementations.
typedef boost::shared_ptr<PreProcessImpl> PreProcessImplPtr;
