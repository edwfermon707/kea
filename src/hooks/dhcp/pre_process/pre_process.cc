...
parseAction(ConstElementPtr option,
    ...
    if (elem) {
        ...
        if (opt_cfg->getAction() != PreProcessImpl::NONE) {
...
const SimpleKeywords PreProcessImpl::OPTION_PARAMETERS = {
...
const SimpleKeywords PreProcessImpl::SUB_OPTION_PARAMETERS = {
    { "code",              Element::integer },
    { "name",              Element::string },
    { "space",             Element::string },
    { "csv-format",        Element::boolean },
...
PreProcessImpl::OptionConfig::OptionConfig(uint16_t code,
                                           OptionDefinitionPtr def)
    : code_(code), def_(def), action_(NONE) {
}

PreProcessImpl::OptionConfig::~OptionConfig() {
}

PreProcessImpl::SubOptionConfig::SubOptionConfig(uint16_t code,
                                                 OptionDefinitionPtr def,
...
PreProcessImpl::SubOptionConfig::~SubOptionConfig() {
}

PreProcessImpl::PreProcessImpl() {
}
...
void
PreProcessImpl::configure(ConstElementPtr options) {
    if (!options) {
...
void
PreProcessImpl::parseOptionConfig(ConstElementPtr option) {
...
void
PreProcessImpl::parseSubOptions(ConstElementPtr sub_options,
...
void
PreProcessImpl::parseSubOption(ConstElementPtr sub_option,
                               OptionConfigPtr opt_cfg,
                               Option::Universe universe) {
    if (!sub_option) {
...
void
PreProcessImpl::logAction(Action action, uint16_t code,
                          const string& value) {
    if (action == NONE) {
        return;
    }
    if (action == REMOVE) {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_REMOVE)
            .arg(code);
        return;
    }
    ostringstream repr;
    ...
    if (action == SUPERSEDE) {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_SUPERSEDE)
            .arg(code)
            .arg(repr.str());
    } else {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_ADD)
            .arg(code)
            .arg(repr.str());
    }
}
...
void
PreProcessImpl::logSubAction(Action action, uint16_t code,
    ...
    if (action == REMOVE) {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_SUB_REMOVE)
            .arg(code)
            .arg(container_code);
        return;
    }
    ...
    if (action == SUPERSEDE) {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_SUB_SUPERSEDE)
            .arg(code)
            .arg(container_code)
            .arg(repr.str());
    } else {
        LOG_DEBUG(pre_process_logger, DBGLVL_TRACE_BASIC,
                  PRE_PROCESS_PROCESS_SUB_ADD)
            .arg(code)
            .arg(container_code)
            .arg(repr.str());
    }
