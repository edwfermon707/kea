...
/// @file This file contains tests which exercise the pkt[46]_receive callouis
/// called by the pre process hook library. In order to test the callouts
...
/// @brief Test fixture for testing callouts called by the pre-process library
...
TEST_F(CalloutTest, pkt4Receive) {
    ...
    // Load the library.
    addLib(PRE_PROCESS_LIB_SO, params);
    loadLibs();
...
TEST_F(CalloutTest, pkt6Receieve) {
    // Prepare load() parameters.
    ElementPtr params = Element::createMap();
    ElementPtr options = Element::createList();
    params->set("options", options);
