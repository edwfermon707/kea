...
1. [ ] If any changes have been done to database schemas, then:
   ...
   * Prepare Release Notes
     ...
     1. [ ] Finish release notes and conduct its review. Also please notify @sgoldlust or @vicky that release notes are ready for review. Due to time difference please do this at least 36 hours before planned release.
...
1. [ ] Run [update-code-for-release.py](https://gitlab.isc.org/isc-private/qa-dhcp/-/blob/master/kea/build/update-code-for-release.py) \
   Example command: `GITLAB_TOKEN='...' ./update-code-for-release.py 1.9.7 --repo-dir ~/isc/repos/kea/` Use `--upload` to commit changes. \
   ...
   1. regenerate parsers using Bison from Docker\
      With `--upload`:
   1. create an issue in GitLab for release changes in kea repo
   1. create branches and merge requests for kea and kea-premium
   1. commit the changes in both repos
   1. checkout created branches in both repos
   1. commit and push the changes to GitLab server
...
Now it's time to publish the code.
 ...
 1. [ ] Mark Jenkins jobs with release artifacts to be kept forever and update description of build by adding there version of released kea (e.g. Kea-2.2.2).
 ...
 1. [ ] Login to repo.isc.org and upload final tarball to public ftp using make-available script (only OPS can remove incorrectly uploaded tarballs).
    * [ ] For premium tarballs use `--private` option.
      ...
      * Example command: `make-available --public --symlink=cur /data/shared/sweng/kea/releases/1.8.2`.
